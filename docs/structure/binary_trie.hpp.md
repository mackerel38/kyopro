...
data:
  ...
  bundledCode: "#line 2 \"structure/binary_trie.hpp\"\n#include <bits/stdc++.h>\n\
    ...
    \ add x (creates nodes only on insert)\n//   erase(x)       -- remove one x (no-op\
    \ if not present, never creates nodes)\n//   count(x)       -- number of x stored\n\
