...
data:
  ...
  bundledCode: "#line 2 \"graph/graphtemplate.hpp\"\n#include<bits/stdc++.h>\nusing\
    ...
    template <class T, bool directed>\nvector<T> dijkstra(graph<T, directed, true>&\
    ...
    \ <class T, bool directed>\npair<vector<T>, vector<int>> dijkstra_prev(graph<T,\
    \ directed, true>& g, int src) {\n    int n = g.size();\n    const T INF = numeric_limits<T>::max()\
    ...
    \    return path;\n}\n"
  code: "#pragma once\n#include \"graphtemplate.hpp\"\n\n// Dijkstra: returns shortest\
    \ distances from src\n// Use: graph<T, directed, true> (weighted=true)\ntemplate\
    \ <class T, bool directed>\nvector<T> dijkstra(graph<T, directed, true>& g, int\
    \ src) {\n    int n = g.size();\n    const T INF = numeric_limits<T>::max() /\
    \ 2;\n    vector<T> dist(n, INF);\n    priority_queue<pair<T,int>, vector<pair<T,int>>,\
    \ greater<pair<T,int>>> pq;\n    dist[src] = T{};\n    pq.push({T{}, src});\n\
    \    while (!pq.empty()) {\n        auto [d, v] = pq.top(); pq.pop();\n      \
